setTimeout(() => {
    ...
    function getToken(){
        return new Promise((resolve, reject)=>{
            var settings = {
                ...
                "data": {
                  "grant_type": "password",
                  "username": "usuarioexterno@millete.com",
    ...
    function createUser(token, tokenType, user){
        return new Promise((resolve, reject)=>{
            var urlLappizUser = `${backandGlobal.api2}/MilleteNomina_Lappiz.api/api/lappiz/Lappiz_Users`;
            var urlReplicate = `${backandGlobal.url}/api/Users/replicate?appCode=MilleteNomina_Lappiz&languageApp=es`;
            var urlGetStatus = `${backandGlobal.url}/api/Users/getStatus?email=${user.email}`;
            
            var settings = {
                "url": "https://tx.lappiz.io/MilleteNomina_Lappiz.api/api/functions/replicateUser",
                "method": "POST",
                "timeout": 0,
                "headers": {
