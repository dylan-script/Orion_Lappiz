...
setTimeout(() => {
  ...
  if (idVista == '981be98e-a078-4759-bcba-0c59e3721812') {
    debugger;
    if (e.isNew) {
      $('#importarClientes').change(function (oEvent) {
        debugger;
        /* beginAuto = true; */
        ...
        if (separador[separador.length - 1] != "xlsx") {
          ...
          reader.onload = function (e) {
            ...
            for (let i = 5; i < Data.length; i++) {
              var id = Data[i]["Número documento Identidad"];

              var consulta = ajaxQuery(
                `SELECT * FROM Orion_Lappiz_Cliente WHERE IdTerceroCliente ='${id}'`
              );
              var IdTblTipoCliente = Data[i]["Tipo Cliente*"];
              var IdTblTipoId = Data[i]["Tipo documento de Identidad"];
              var x
              ...
              if (consulta.length == 0) {
                ...
                Query += ` INSERT INTO Orion_Lappiz_Cliente (IdTblTipoCliente, IdTblTipoId, Nombre1, Nombre2, Apellido1, Apellido2, RazonSocial, Direccion1, Direccion2, CodigoPostal, Email, Telefono1, Telefono2)
                                    VALUES ('${IdTblTipoCliente}',
                                            '${IdTblTipoId}',
                                            '${Nombre1}',
                                            '${Nombre2}',
                                            '${Apellido1}',
                ...
                var cambios = 0;
                var update = ``;
                if (IdTblTipoCliente != consulta[0].IdTblTipoCliente) {
                  update += ` IdTblTipoCliente = '${CENombreCuentaCont}', `;
                  cambios++;
                }
                ...
                debugger
                if (cambios > 0) {
                  Query += ` UPDATE Orion_Lappiz_Cliente
                            			SET ${newUpdate}
